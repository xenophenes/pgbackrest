...
sub process
{
    ...
    if (optionTest(OPTION_OUTPUT, INFO_OUTPUT_TEXT))
    {
        ...
        {
            syswrite(*STDOUT, 'No stanzas exist in ' . $oFile->pathGet(PATH_REPO) . ".\n");
        }
    }
...
sub stanzaList
{
    ...
    {
        my @stryStanza = $oFile->list(PATH_REPO . qw{/} . CMD_BACKUP, {bIgnoreMissing => true});
        ...
        foreach my $strStanzaFound (@stryStanza)
        {
            ...
            if (defined($hDbInfo->{&INFO_KEY_ID}) && defined($hDbInfo->{&INFO_KEY_VERSION}))
            {
                ...
                if ($oFile->exists(PATH_REPO . "/${strArchivePath}"))
                {
                    my @stryWalMajor = $oFile->list(PATH_REPO . "/${strArchivePath}", {strExpression => '^[0-F]{16}$'});

                    # Get first WAL segment
                    foreach my $strWalMajor (@stryWalMajor)
                    {
                        my @stryWalFile = $oFile->list(
                            PATH_REPO . "/${strArchivePath}/${strWalMajor}",
...
sub backupList
{
    ...
    my $oBackupInfo = new pgBackRest::BackupInfo($oFile->pathGet(PATH_REPO . qw{/} . CMD_BACKUP . "/${strStanza}"), false, false);

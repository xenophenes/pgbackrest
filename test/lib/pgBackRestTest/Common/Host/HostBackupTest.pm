...
sub backupEnd
{
    ...
    if ($strType eq BACKUP_TYPE_FULL || $self->hardLink())
    {
        my $hTablespaceManifest = $self->{oFile}->manifest(
            PATH_REPO_BACKUP . "/${strBackup}/" . MANIFEST_TARGET_PGDATA . '/' . DB_PATH_PGTBLSPC);
    ...
    elsif ($self->{oFile}->exists(PATH_REPO_BACKUP . "/${strBackup}/" . MANIFEST_TARGET_PGDATA . '/' . DB_PATH_PGTBLSPC))
    {
    ...
    my $strLatestLink = $self->{oFile}->pathGet(PATH_REPO_BACKUP . qw{/} . LINK_LATEST);
    ...
    if (defined($self->{oLogTest}) && (!defined($$oParam{bSupplemental}) || $$oParam{bSupplemental}))
    {
        ...
        {
            $self->{oLogTest}->supplementalAdd($self->{oFile}->pathGet(PATH_REPO_BACKUP . "/${strBackup}/" . FILE_MANIFEST));
...
sub backupCompare
{
    ...
    my $oActualManifest = new pgBackRest::Manifest(
        $self->{oFile}->pathGet(PATH_REPO_BACKUP . "/${strBackup}/" . FILE_MANIFEST));
        ...
        if ($oExpectedManifest->{&MANIFEST_SECTION_BACKUP_OPTION}{&MANIFEST_KEY_COMPRESS})
        {
            my $lRepoSize =
                $oActualManifest->test(MANIFEST_SECTION_TARGET_FILE, $strFileKey, MANIFEST_SUBKEY_REFERENCE) ?
                    ...
                    (fileStat($self->{oFile}->pathGet(PATH_REPO_BACKUP . "/${strBackup}/${strFileKey}.gz")))->size;
...
sub backupLast
{
    ...
    my @stryBackup = $self->{oFile}->list(
        PATH_REPO_BACKUP, {strExpression => '[0-9]{8}-[0-9]{6}F(_[0-9]{8}-[0-9]{6}(D|I)){0,1}', strSortOrder => 'reverse'});
...
sub manifestMunge
{
    ...
    $self->infoMunge($self->{oFile}->pathGet(PATH_REPO_BACKUP . "/${strBackup}/" . FILE_MANIFEST), $hParam, $bCache);
...
sub manifestRestore
{
    ...
    $self->infoRestore($self->{oFile}->pathGet(PATH_REPO_BACKUP . "/${strBackup}/" . FILE_MANIFEST), $bSave);

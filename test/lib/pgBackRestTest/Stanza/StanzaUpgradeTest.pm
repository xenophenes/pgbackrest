...
sub run
{
    ...
    for (my $bRemote = false; $bRemote <= true; $bRemote++)
    {
        ...
        #--------------------------------------------------------------------------------------------------------------------------
        $oHostBackup->executeSimple('rm ' . $oFile->pathGet(PATH_REPO_BACKUP . qw{/} . FILE_BACKUP_INFO));
        ...
        $self->testResult(
            sub {$oFile->list(PATH_REPO_ARCHIVE . qw{/} . PG_VERSION_94 . '-2/0000000100000001')},
        ...
        $oHostBackup->executeSimple('rm ' . $oFile->pathGet(PATH_REPO_ARCHIVE . qw{/} . ARCHIVE_INFO_FILE));
        ...
        $oHostBackup->executeSimple('mkdir ' . $oFile->pathGet(PATH_REPO_ARCHIVE) . '/' . '10.0-3');
        $oHostBackup->executeSimple('mkdir ' . $oFile->pathGet(PATH_REPO_ARCHIVE) . '/' . '10.0-3/0000000100000001');
        $oHostBackup->executeSimple('cp ' . $self->dataPath() . '/backup.wal1_' . WAL_VERSION_92 . '.bin '
            . $oFile->pathGet(PATH_REPO_ARCHIVE) . '/' . '10.0-3/0000000100000001/000000010000000100000001');

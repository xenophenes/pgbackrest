...
sub run
{
    ...
    if ($self->begin("Stanza::upgradeCheck"))
    {
        ...
        $self->testResult(sub {$oStanza->upgradeCheck($oBackupInfo, PATH_REPO_BACKUP, ERROR_BACKUP_MISMATCH)}, true,
            'backup upgrade needed');
        $self->testResult(sub {$oStanza->upgradeCheck($oArchiveInfo, PATH_REPO_ARCHIVE, ERROR_ARCHIVE_MISMATCH)}, false,
        ...
        $self->testResult(sub {$oStanza->upgradeCheck($oArchiveInfo, PATH_REPO_ARCHIVE, ERROR_ARCHIVE_MISMATCH)}, true,
        ...
        $self->testResult(sub {$oStanza->upgradeCheck($oBackupInfo, PATH_REPO_BACKUP, ERROR_BACKUP_MISMATCH)}, false,
        ...
        $self->testException(sub {$oStanza->upgradeCheck($oArchiveInfo, PATH_REPO_ARCHIVE, ERROR_ASSERT)}, ERROR_ARCHIVE_MISMATCH,
        ...
        $self->testException(sub {$oStanza->upgradeCheck($oBackupInfo, PATH_REPO_BACKUP, ERROR_ASSERT)}, ERROR_BACKUP_MISMATCH,

...
sub archiveCheck
{
    ...
    do
    {
        $bFound = $oStorage->exists(PATH_REPO_ARCHIVE . "/${strArchiveCheck}");
    }
    while (!$bFound && waitMore($oWait));

    if (!$bFound)
    {
        confess 'unable to find ' . $oStorage->pathGet(PATH_REPO_ARCHIVE . "/${strArchiveCheck}");
    }

    if (defined($strSpoolPath))
    {
        $oStorage->remove("${strSpoolPath}/archive/" . $self->stanza() . "/out/${strArchiveFile}.ok");
    }
}
...
sub run
{
    ...
    for (my $bArchiveAsync = false; $bArchiveAsync <= true; $bArchiveAsync++)
    {
        ...
        $oStorage->pathCreate("${strXlogPath}/archive_status");
        my $strArchiveFile1 = $self->walGenerate($oStorage, $strXlogPath, WAL_VERSION_94, 1, $strSourceFile1);
        ...
        $oStorage->remove($oHostDbMaster->spoolPath() . '/archive/' . $self->stanza() . "/out/${strSourceFile1}.error")
            if $bArchiveAsync;
        ...
        for (my $iBackup = 1; $iBackup <= 3; $iBackup++)
        {
            ...
            for (my $iArchive = 1; $iArchive <= $iArchiveMax; $iArchive++)
            {
                ...
                $strArchiveFile = $self->walGenerate($oStorage, $strXlogPath, WAL_VERSION_94, 2, $strSourceFile);
                ...
                if ($iBackup == 1 && $iArchive == 2)
                {
                    ...
                    $strArchiveTmp =
                        ...
                        ($bCompress ? ".$oStorage->{strCompressExtension}" : '') . '.pgbackrest.tmp';

                    executeTest('sudo chmod 770 ' . dirname($strArchiveTmp));
                    $oStorage->put($strArchiveTmp, {strContent => 'JUNK'});
                ...
                push(
                    ...
                    ($bCompress ? ".$oStorage->{strCompressExtension}" : ''));
                ...
                if (defined($strArchiveTmp))
                {
                    ...
                    do
                    {
                        $bFound = $oStorage->exists($strArchiveTmp);
                    }
                    while ($bFound && waitMore($oWait));
                ...
                if ($iArchive == $iBackup)
                {
                    $oStorage->remove($oHostDbMaster->spoolPath() . '/archive/' . $self->stanza() . "/out/${strSourceFile}.ok")
                        if $bArchiveAsync;
                    ...
                    $oStorage->remove($oHostDbMaster->spoolPath() . '/archive/' . $self->stanza() . "/out/${strSourceFile}.error")
                        if $bArchiveAsync;
                    ...
                    $strArchiveFile = $self->walGenerate($oStorage, $strXlogPath, WAL_VERSION_94, 1, $strSourceFile);
                    ...
                    $strArchiveFile = $self->walGenerate($oStorage, $strXlogPath, WAL_VERSION_94, 2, "${strSourceFile}.partial");
                    $oHostDbMaster->executeSimple(
                        $strCommand . " --no-" . OPTION_REPO_SYNC . " ${strXlogPath}/${strSourceFile}.partial",
                        {oLogTest => $self->expect()});
                    $self->archiveCheck(
                        $oStorage, "${strSourceFile}.partial", $strArchiveChecksum, $bCompress,
                    ...
                    $strArchiveFile = $self->walGenerate($oStorage, $strXlogPath, WAL_VERSION_94, 1, "${strSourceFile}.partial");
                    $oHostDbMaster->executeSimple(
                ...
                {
                    $self->archiveCheck(
                        $oStorage, $strSourceFile, $strArchiveChecksum, $bCompress,
                        $bArchiveAsync ? $oHostDbMaster->spoolPath() : undef);
                }
            }
        }
        ...
        #---------------------------------------------------------------------------------------------------------------------------
        $self->testResult(
            sub {$oStorage->list(PATH_REPO_ARCHIVE . qw{/} . PG_VERSION_94 . '-1/0000000100000001')},
        ...
        if (defined($self->expect()))
        {
            ...
            $self->expect()->supplementalAdd($oStorage->pathGet(PATH_REPO_ARCHIVE . qw{/} . ARCHIVE_INFO_FILE));
        }
    }
    }
    }

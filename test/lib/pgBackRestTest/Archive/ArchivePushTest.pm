...
sub archiveCheck
{
    ...
    do
    {
        $bFound = $oFile->exists(PATH_REPO_ARCHIVE . "/${strArchiveCheck}");
    }
    while (!$bFound && waitMore($oWait));

    if (!$bFound)
    {
        confess 'unable to find ' . $oFile->pathGet(PATH_REPO_ARCHIVE . "/${strArchiveCheck}");
    }
...
sub run
{
    ...
    for (my $bArchiveAsync = false; $bArchiveAsync <= true; $bArchiveAsync++)
    {
        ...
        #---------------------------------------------------------------------------------------------------------------------------
        $self->testResult(
            sub {$oFile->list(PATH_REPO_ARCHIVE . qw{/} . PG_VERSION_94 . '-1/0000000100000001')},
        ...
        if (defined($self->expect()))
        {
            ...
            $self->expect()->supplementalAdd($oFile->pathGet(PATH_REPO_ARCHIVE . qw{/} . ARCHIVE_INFO_FILE));
        }
    }
    }
    }
